/*============================================================
	...
	File Name:			submitted_jqxgrid_load.js
=============================================================*/

function LoadSubmittedGrid() {
	
	var dataFieldsArray = [];
	
	var jqxhrFormData = $.getJSON(ruIP + ruPort + listsDB + listEN + "read/forms/FormData?where=\"IsLocked='1' AND SiteGUID = '"+UserData[0].SiteGUID+"' ORDER BY CreatorGUID, FormGUID, DateCreated DESC\"", function() {
		...
		var jqxhrPersonData = $.getJSON(ruIP + ruPort + listsDB + listEN + "read/cfg/Person?where=\"SiteGUID = '"+UserData[0].SiteGUID+"'\"", function() {
			...
			var jqxhrForms = $.getJSON(ruIP + ruPort + listsDB + listEN + "read/virtual/"+UserData[0].SiteGUID+"/Forms", function() {
				...
				GenerateSubmittedGrid(dataFieldsArray);
			});
		});
	});
}

function GenerateSubmittedGrid(dataFieldsArray) {
	
	($("#submitted_jqxgrid").length != 0)  ? $('#submitted_jqxgrid').jqxGrid('destroy') : false;	
	$("#modal-table .modal-subcontent").append("<div id='submitted_jqxgrid'></div>");
	...
	var cellsrenderer = function (row, columnfield, value, defaulthtml, columnproperties) {
		...
		createdTime    =  "<div style='margin-top:5px'>&nbsp;" + monthNames[month] + " " + day + ", " + year + "</div>"; 
	...
	$("#submitted_jqxgrid").jqxGrid({
		...
		columns: [
		  ...
		  { text: 'DateLocked',			datafield: 'DateLocked',		hidden: false, cellsrenderer: formatTime },
		  { text: 'DateSynced',			datafield: 'DateSynced',		hidden: false, cellsrenderer: formatTime },
		  { text: 'IsLocked',			datafield: 'IsLocked',			hidden: true }
		]
	});
				
	$('#submitted_jqxgrid').on('rowselect', function (event) {
		// event arguments.
		var args = event.args;
		// row's bound index.
